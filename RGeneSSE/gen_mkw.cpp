...
int DB_WriteFromDatabaseToFile(std::string db_file, std::vector<std::string> db_kw, std::vector<std::set<unsigned int>> db, KW_TYPE dtype = KW_TYPE::HEX)
{
    ...
    std::cout<<"Total number of unique (mkw-id) pairs = "<< count << std::endl;
...
{	
	...
	std::cout << "Meta-keyword toiri hoye gache! " << db_mkw.size() << std::endl;
	...
	for(auto m:db_mkw)
	{
		...
		j++;
		// std::cout << j << " Meta-keyword toiri hoye gache!" << std::endl;
	}
	
	cout<< "  MetaDB Size = "<< meta_db.size() << "\n";
