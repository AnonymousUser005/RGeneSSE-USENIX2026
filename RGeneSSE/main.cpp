...
int read_data(const string& filename, Node*& root, unsigned long& minId, unsigned long& maxId, map<unsigned long, string>& db6kMap) {
    ...
    while (getline(file, line)) {
        ...
        id = stoul(keyword, nullptr, 16);
        db6kMap[id] = keyword;
        ids.insert(id); // Store the ID in the set

        // cout << "idStr: " << idStr << ",  id: " << id << ", keyword: " << keyword << std::endl; 
...
void collect_and_write_ancestors(Node* node, ofstream& file,map<unsigned long, string>&db6kMap) {
    if (node == NULL) return;
...
void outputNodeDetailsToTxt(Node* node, const string& filename,map<unsigned long, string>& db6kMap) {
    ofstream file(filename);
    collect_and_write_ancestors(node,file,db6kMap);
    file.close();
}
void print_map(map<unsigned long, string>&db6kMap){
    for(auto i:db6kMap){
...
int main()
{
	...
	std::string raw_db_file_for_tree = "../../databases/vcf5k.csv";
    // std::string raw_db_file_for_tree = "../../Range-SSE/vcf100k.csv";
	std::string meta_db_file = "../../databases/meta_vcf5k.dat";
    ...
    Node* root = nullptr;
    map<unsigned long, string> db6kMap;
    ...
    // vector<string> ans = perform_range_search(bestRangeCover);
    // for(auto i:ans){
    //     cout<<i<<" ";
    // }
	...
	cout<<"[*] Rawdb generated: " << raw_db.size() << std::endl;
