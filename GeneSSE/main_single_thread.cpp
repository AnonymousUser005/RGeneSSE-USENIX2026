...
string widxdb_file = "./meta_vcf10k.dat";           //VCF Database
string subdir_name;
...
int GeneSSE_SetUp_Top()
{
    cout << "Executing GeneSSE setup..." << endl;
    ...
    if(filesystem::exists(update_count_file)){
        ...
        for(auto& row:updates){
            ...
            for(auto mkw:mkws){
                GeneSSE_Update(intToStr(mkw), kw, id, 0x01);
